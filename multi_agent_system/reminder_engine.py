...
class ReminderEngine:
    ...
    def _email_subject_day14(self, inv: InvoiceRecord) -> str:
        return (
            f"Formal Payment Notice — Invoice {inv.invoice_id} (14 Days Overdue)"
        )

    def _email_body_day14(self, inv: InvoiceRecord) -> str:
        return f"""Dear {inv.client_name},

This is a formal notice that invoice {inv.invoice_id} for {inv.project_name} remains unpaid.

Amount due: Rs {inv.total_amount:,.2f}
Original due date: {inv.due_date}
Days overdue: 14

Please find the formal invoice attached to this email.
Prompt payment would be greatly appreciated.

If you have already made payment, please disregard this notice.

Regards,
The {inv.project_name} Team
"""
...
"""
    ...
    def check_and_send(
        ...
        if days >= 14:
            ...
            # IMPORTANT: keep email body plain-text only. We still provide an
            # HTML wrapper for SendGrid, but it must NOT include the invoice HTML inline.
            body_html = f"<pre style='font-family:Arial,sans-serif;font-size:13px;white-space:pre-wrap'>{body_text}</pre>"
            pdf_bytes = None
            if _WEASYPRINT_OK:
                try:
